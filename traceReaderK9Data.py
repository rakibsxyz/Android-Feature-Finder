...
def packageNameAndMethodNamePerser(line):
    ...
    if(len(methodName) > 1):
        ...
        className = activityName 
    if len(methodName) > 0: 
        totalString = pkgname + activityName + methodName[0]
    else:
        totalString = pkgname + activityName
            ...
            if "java.lang.Throwable" in lines[i]:
                ...
                if i > 0:
                    if(len(activityNameList) == 0):
                        activityNameList.append("a")
                    if(len(fragmentNameList) == 0):
                        fragmentNameList.append("f")
                    if(len(classNameList) == 0):
                        classNameList.append("c")

                    if(len(methodNameList) == 0):
                        methodNameList.append("m")
                    if(len(methodStack) == 0):
                        methodStack.append("ms")
                    if(len(allPackageName) == 0):
                        allPackageName.append("ap")
                ...
                if "com." in lines[i]:
                    allPackageName.append(packageNameAndMethodNamePerser(lines[i]))
...
# readTraceAndParse(traceFileName)
